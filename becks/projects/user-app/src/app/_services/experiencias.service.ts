        ...
        if (response.status >= 200 && response.status < 300) {
          ...
          response.body.forEach((element, index) => {
            const elementoResponse: Exp = {
              ...
              detalleExp: false,
              imagesExp: ( urlServer + environment.user.getImgExp + element.id + "_desk" + "?time_stamp=" + Math.floor(Date.now() / 1000) ),
              imagesExpMob: ( urlServer + environment.user.getImgExp + element.id + "_mob" + "?time_stamp=" + Math.floor(Date.now() / 1000) ),
